...
void test_process_entry(uint64_t argc, char *argv[]) {
	...
	if (argc > 0 && argv != NULL && argv[0] != NULL && argv[0][0] != '\0') {
		...
		if (process_count <= 0 || process_count > 64) {
			print_format("Invalid process count (1-64). Using default: 3\n");
			process_count = 3;
		}
	}

	print_format("Starting test_process with %d processes...\n\n", process_count);
...
void test_mm_entry(uint64_t argc, char *argv[]) {
	...
	char *mm_argv[] = {NULL};
	if (argc > 0 && argv != NULL && argv[0] != NULL && argv[0][0] != '\0') {
		print_format("Running test_mm with custom size: %s bytes\n", argv[0]);
		mm_argv[0] = argv[0];
	}
	else {
		print_format("Running test_mm with default size: 1MB\n");
