...
void pvpMode() {
	...
	while (crashed == 0) {
		...
		if (!isValidPos2J(snk1[len1][X] + direc[dir][X], snk1[len1][Y] + direc[dir][Y], snk1, len1, cola, snk2, len2,
						  cola2)) {
			loser = 1;
			crashed = 1;
		}
		if (!isValidPos2J(snk2[len2][X] + direc[dir2][X], snk2[len2][Y] + direc[dir2][Y], snk2, len2, cola2, snk1, len1,
						  cola)) {
			loser += 2;
			crashed = 2;
		}
