...
uint8_t *getFontChar(char c) {
	if (c < MIN_CHAR || c > MAX_CHAR) {
		return NULL;
		// return fontBitmap + ('?' - MIN_CHAR) * font_height; // Si el char no se encuentra, retornamos un '?'
	}
	return fontBitmap + (c - MIN_CHAR) * font_height;
}

// Función que recibe un Color (del enum) y devuelve el código hexadecimal correspondiente
uint32_t getColor(Color color) {
	switch (color) {
		...
		default:
			return 0xFFFFFF; // Valor por defecto (Blanco)
	}
}
