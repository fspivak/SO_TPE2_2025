...
int memory_free(MemoryManagerADT self, void *ptrs)
{
    ...
    if (buddy_block == NULL || (uint64_t)buddy_block < self->managed_memory_start || (uint64_t)buddy_block >= MEMORY_END) {
           ...
           buddy_block->order == block->order)
    {
        ...
        buddy_block = (block_t *)(block_t *)(self->managed_memory_start + (((uint64_t)block_pos) ^ ((1L << block->order))));
